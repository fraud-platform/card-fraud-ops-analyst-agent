...
def compute_insight_severity(pattern_scores: list[Any], similarity_result: Any) -> str:
    """Compute insight severity from pattern and similarity scores."""
    score_values = [
        float(s.get("score", 0.0)) if isinstance(s, dict) else float(getattr(s, "score", 0.0))
        for s in pattern_scores
    ]
    pattern_avg = sum(score_values) / len(score_values) if score_values else 0.0

    combined = (pattern_avg * 0.7) + (_similarity_overall(similarity_result) * 0.3)
