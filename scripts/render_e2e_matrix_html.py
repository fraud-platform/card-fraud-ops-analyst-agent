...
def _render_rows(report: dict[str, Any]) -> str:
    ...
    for row in rows:
        ...
        body.append(
            ...
            f"<td>{html.escape(str(row.get('reasoning_llm_outcome', '')))}</td>"
            f"<td>{html.escape(str(row.get('reasoning_tool_status', '')))}</td>"
    ...
    return (
        ...
        "<th>Reasoning LLM</th><th>Reasoning Tool</th><th>Recs</th><th>Evidence</th>"
        "<th>Run ms</th><th>Detail ms</th><th>Issues</th>"
        "</tr></thead>"
